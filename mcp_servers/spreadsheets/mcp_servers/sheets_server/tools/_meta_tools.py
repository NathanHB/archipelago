...
class FilterTabResult(BaseModel):
...
WRONG: 'set_value', 'add_rows' - these will fail!""",
    ...
    filters: list[dict[str, Any]] | None = Field(
        None,
        description=(
            "Filter conditions for 'filter_tab'. Each condition: "
            "{column: 'A' or header name, operator: 'equals'|'contains'|'greater_than'|etc., value: ...}. "
...
SHEETS_HELP = HelpResponse(
    ...
    actions={
        ...
        "filter_tab": ActionInfo(
            ...
            optional_params=["cell_range", "match_all", "use_headers"],
        ),
    },
)
...
async def sheets(request: SheetsInput) -> SheetsOutput:
    """Manage .xlsx spreadsheets: create, read tabs/CSVs, edit cells, charts.

    Actions: help | create | delete | read_tab | read_csv | list_tabs |
             add_tab | delete_tab | edit | add_content | delete_content | create_chart

    Paths must start with '/' (e.g., '/data/budget.xlsx').
    ...
    match request.action:
        ...
        case "filter_tab":
            ...
            result = await _filter_tab(
                ...
                request.filters,
                request.cell_range,
                request.match_all if request.match_all is not None else True,
                request.use_headers if request.use_headers is not None else True,
            )
            if _is_read_error(result):
