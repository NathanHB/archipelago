...
# Filter operators for filtering spreadsheet data
_FILTER_OPERATORS_CANONICAL = [
    "equals",
    "not_equals",
    "greater_than",
    "less_than",
...
class FilterCondition(BaseModel):
    ...
    column: str  # Column letter (A, B, etc.) or header name
    operator: str  # Filter operator
    value: Any | None = (
        None  # Value to compare against (optional for is_empty/is_not_empty)
    )
    ...
    def _validate_operator(cls, value: str) -> str:
        lower_value = value.lower()
        if lower_value not in FILTER_OPERATORS_MAP:
            raise ValueError(
                f"operator must be one of: {sorted(_FILTER_OPERATORS_CANONICAL)}"
            )
        return FILTER_OPERATORS_MAP[lower_value]
