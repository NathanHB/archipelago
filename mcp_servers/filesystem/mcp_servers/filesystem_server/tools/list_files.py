...
def list_files(
    path: Annotated[
        str,
        Field(
            description="Directory path to list (relative to sandbox root, not system filesystem). Default: '/' (sandbox root). Example: /documents"
        ),
    ] = "/",
) -> str:
    """List files and folders in the given path. Paths are relative to the sandbox root, not the system filesystem."""
